...
export const SongsSection: React.FC<SongsSectionProps> = () => {
  ...
  return (
    <div className="relative w-full h-full min-h-screen max-h-screen flex flex-col justify-between items-center px-3 sm:px-8 py-3 sm:py-8 overflow-hidden select-none bg-paper/60">
      ...
      <div className="relative z-10 w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-2.5 sm:gap-6 my-auto px-1 sm:px-2 overflow-hidden">
        {/* ── CARD 1: ART GALLERY (Her First Song to Him) ── */}
        <motion.div
          initial={{ opacity: 0, x: -30, y: 20 }}
          animate={{ opacity: 1, x: 0, y: 0 }}
          transition={{ duration: 0.8, delay: 0.15, ease: [0.16, 1, 0.3, 1] }}
          className="relative bg-paper-card border border-paper-border rounded-2xl sm:rounded-3xl p-3 sm:p-6 shadow-paper-float flex flex-col justify-between group overflow-hidden"
        >
          {/* Top Washi Tape */}
          ...
          <div>
            {/* Tag Badge */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-pastel-blush/70 border border-maroon/20 text-maroon text-[11px] font-bold font-sans tracking-wide mb-2 sm:mb-3">
              <Heart className="w-3 h-3 fill-maroon text-maroon" />
              <span>{songs.track1.tag}</span>
            </div>

            {/* Vinyl Record Sleeve Preview Area */}
            <div className="relative flex items-center justify-between gap-3 sm:gap-4 p-2.5 sm:p-3.5 bg-paper rounded-xl sm:rounded-2xl border border-paper-border/80 shadow-inner mb-2 sm:mb-3.5">
              ...
              <div className="relative w-14 h-14 sm:w-20 sm:h-20 shrink-0 flex items-center justify-center">
                ...
                >
                  <DoodleVinylRecord className="w-14 h-14 sm:w-20 sm:h-20" isSpinning={activeTrack === 'track1'} />
                </motion.div>

                {/* Sleeve Cover Face */}
                <div className="relative z-10 w-14 h-14 sm:w-20 sm:h-20 rounded-lg sm:rounded-xl bg-pastel-blush/80 border border-maroon/30 shadow-md flex flex-col items-center justify-center p-1.5 sm:p-2 text-center overflow-hidden">
              ...
              <div className="flex-1 min-w-0 pr-2">
                <h3 className="font-serif font-bold text-sm sm:text-lg text-maroon leading-snug line-clamp-2">
                  {/* [SONG_1_TITLE] */}
                  {songs.track1.title}
                </h3>
            ...
            <p className="text-xs sm:text-sm font-hand text-ink leading-relaxed px-1 line-clamp-2 sm:line-clamp-none">
              "{songs.track1.caption}"
            </p>
          </div>

          {/* Action Buttons (Play Preview + Spotify Link) */}
          <div className="flex items-center justify-between gap-2 sm:gap-3 mt-2.5 sm:mt-4 pt-2.5 sm:pt-3.5 border-t border-paper-border/70">
            {/* Play/Pause Button */}
            <button
              onClick={() => handleTogglePlay('track1')}
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-mono font-bold transition-all shadow-tactile cursor-pointer hover:scale-105 active:scale-95 ${
                activeTrack === 'track1'
        ...
        >
          ...
          <div>
            {/* Tag Badge */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-pastel-butter/70 border border-maroon/20 text-maroon text-[11px] font-bold font-sans tracking-wide mb-2 sm:mb-3">
              <Heart className="w-3 h-3 fill-maroon text-maroon" />
            ...
            <div className="relative flex items-center justify-between gap-3 sm:gap-4 p-2.5 sm:p-3.5 bg-paper rounded-xl sm:rounded-2xl border border-paper-border/80 shadow-inner mb-2 sm:mb-3.5">
              ...
              <div className="relative w-14 h-14 sm:w-20 sm:h-20 shrink-0 flex items-center justify-center">
                ...
                <motion.div
                  ...
                  className="absolute inset-0 z-0"
                >
                  <DoodleVinylRecord className="w-14 h-14 sm:w-20 sm:h-20" isSpinning={activeTrack === 'track2'} />
                </motion.div>

                {/* Sleeve Cover Face */}
                <div className="relative z-10 w-14 h-14 sm:w-20 sm:h-20 rounded-lg sm:rounded-xl bg-pastel-butter/90 border border-maroon/30 shadow-md flex flex-col items-center justify-center p-1.5 sm:p-2 text-center overflow-hidden">
                  <div className="text-[10px] font-mono font-bold text-maroon uppercase leading-tight">
                    REED
                  </div>
