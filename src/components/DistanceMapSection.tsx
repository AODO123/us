...
export const DistanceMapSection: React.FC<DistanceMapSectionProps> = () => {
  ...
  return (
    <div className="relative w-full h-full min-h-screen max-h-screen flex flex-col justify-between items-center px-3 sm:px-8 py-3 sm:py-10 overflow-hidden select-none bg-paper/70">
      ...
      >
        ...
        <h2 className="text-xl sm:text-5xl font-serif font-bold text-maroon tracking-tight leading-tight flex items-center justify-center gap-1.5 sm:gap-3.5">
          <span>{me.city}</span>
          <Heart className="w-4 h-4 sm:w-8 sm:h-8 fill-maroon text-maroon inline" />
          <span>{her.city}</span>
        </h2>
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-4xl bg-paper-card border border-paper-border rounded-2xl sm:rounded-3xl p-3 sm:p-7 shadow-paper-float overflow-hidden my-auto flex flex-col justify-between"
      >
        {/* Washi Tape accents */}
        ...
        <div className="relative w-full h-32 sm:h-64 md:h-72 rounded-xl sm:rounded-2xl bg-paper border border-paper-border/60 overflow-hidden flex items-center justify-center p-1.5 sm:p-2 shadow-inner">
          ...
          <motion.div
            ...
            className="absolute top-2 sm:top-3 left-1/2 -translate-x-1/2 z-20 px-2.5 sm:px-4 py-1 sm:py-1.5 bg-paper-card/95 backdrop-blur-md rounded-lg sm:rounded-xl border border-maroon/30 shadow-tactile text-center"
          >
            <div className="flex items-center justify-center text-[10px] sm:text-xs font-mono font-bold text-maroon tracking-wider">
              {/* [DISTANCE_KM] */}
              <span>{approxDistanceKm.toLocaleString()} KM APART</span>
            </div>
            <div className="flex items-center justify-center gap-1 text-[10px] sm:text-[11px] font-hand text-ink-muted mt-0.5">
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-4 mt-2.5 sm:mt-4 pt-2.5 sm:pt-4 border-t border-paper-border/70 items-center">
          {/* Costa (Cairo) */}
          <motion.div
          ...
          >
            ...
            <div className="relative shrink-0">
              <div className="w-10 h-10 sm:w-16 sm:h-16 rounded-full p-0.5 sm:p-1 bg-paper border-2 border-maroon/40 shadow-sm overflow-hidden">
                {/* [ASSET_ME2] */}
                <img
            ...
            {/* Label */}
            <div>
              <div className="font-serif font-bold text-xs sm:text-lg text-maroon leading-tight">
                {me.city}, {me.country}
              </div>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6, delay: 0.3 }}
            className="flex items-center justify-end gap-2 sm:gap-3 text-right"
          >
            {/* Label */}
            <div>
              <div className="font-serif font-bold text-xs sm:text-lg text-maroon leading-tight">
